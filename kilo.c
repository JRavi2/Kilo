...
typedef struct erow  {
	int size;
	int rsize;
	// The Actual characters read from a file
	char *chars;
...
void editorSave() {
	if (E.filename == NULL) {
		E.filename = editorPrompt("Enter File Name: %s", NULL);
		if (E.filename == NULL) {
	...
	free(buf);
	editorSetStatusMessage("Can't save to disk! I/O error: %s", strerror(errno));
}
...
void editorFind() {
	...
	if (query)
		...
		// Return to previous cursor position if ESC is pressed
		// i.e. query is NULL
		E.cx = saved_cx;
		E.cy = saved_cy;
	...
	switch (c) {
		...
		case CTRL_KEY('q'):
			if (E.dirty && quit_times > 0) {
				editorSetStatusMessage("WARNING!! Unsaved Changes Present. Press Ctrl-q %d more times to quit", quit_times);
				quit_times--;
				return;
			}
			write(STDOUT_FILENO, "\x1b[2J", 4);
